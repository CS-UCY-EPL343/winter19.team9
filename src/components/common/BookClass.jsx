...
class BookClass extends Component {
    ...
    handleClass = (e) => {
        this.setState({[e.target.name]: e.target.value === 'Select...' ? '' : e.target.value}, () => {
            this.setState({Day: this.state.Day !== null ? [] : this.state.Day}, () => {
                this.setState({Time: this.state.Time !== null ? [] : this.state.Time}, () => {
                    this.setState({CoachName: this.state.CoachName !== null ? [] : this.state.CoachName}, () => {
                        getClassDay(this.state.SelectedClass).then(response => {
                            this.setState({Day: response}, () => {
                                console.clear();
                                console.log("Class = " + this.state.SelectedClass + " & Days dropdown: ");
                ...
                this.setState({CoachName: []}, () => {
                    getClassTime(this.state.SelectedClass, this.state.SelectedDay).then(response => {
                        this.setState({Time: response}, () => {
                            console.log("Class = " + this.state.SelectedClass + ", day = " + this.state.SelectedDay + " & Times dropdown: ");
            ...
            this.setState({CoachName: []}, () => {
                getClassCoach(this.state.SelectedClass, this.state.SelectedDay, this.state.SelectedTime).then(response => {
                    this.setState({CoachName: response}, () => {
                        console.log("Class = " + this.state.SelectedClass + ", day = " + this.state.SelectedDay + ", time = " + this.state.SelectedTime + " & Coaches dropdown: ");
        ...
        this.setState({[e.target.name]: e.target.value === 'Select...' ? '' : e.target.value}, () => {
            getClassID(this.state.SelectedClass, this.state.SelectedDay, this.state.SelectedTime, this.state.SelectedCoach).then(response => {
                this.setState({ClassID: response.data.ClassID.ClassID}, () => {
                    console.log("FINAL SELECTION:\nClass = " + this.state.SelectedClass + ", day = " + this.state.SelectedDay + ", time = "+ this.state.SelectedTime + " & coach = "+ this.state.SelectedCoach);
                });
            });
        });
    ...
    onDelete = (e) => {
        ...
        this.setState({flag: false},
            () => {
                this.props.getSelections(this.state.DayCode, this.state.TimeCode, false, this.state.ClassID, this.state.SelectedClass);
            });
        ...
        return (
            <div className = "col-lg-4 col-md-12 col-sm-12">
                <form className = "form" id = "backBox">
                    ...
                    <div className = "row">
                        <div className= "col-md-6 RowBlock">
                            <button type = "" className="RowButton" onClick={this.onSubmit}>Enroll</button>
                        </div>
                        <div className= "col-md-6 RowBlock">
